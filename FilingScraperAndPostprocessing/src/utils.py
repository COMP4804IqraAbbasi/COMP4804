...
"""The following are the command line arguments that can be passed
"""
...
parser.add_argument('--write_sql', default=True, help='Save metadata to sqlite database? Y/N')
...
parser.add_argument('--report_period', help='search pattern for company report dates')
...
if args.storage:
    ...
    args.storage = path.join(project_dir, 'output_files')
...
args.filings = args.filings or \
    input('Enter filings search text (default: 10-K and 10-Q): ') or \
    '10-K,10-Q'
...
args.start = int(args.start or \
    input('Enter start date for filings search (default: earliest possible date) in YYMMDD : ') or \
...
args.end = int(args.end or \
    input('Enter end date for filings search (default: today`s date) in YYMMDD :') or \
...
if str(args.report_period).lower() == 'all':
    ...
    date_search_string = str(
        ...
        input('Enter filing report period  (default: all periods): ') or
        '.*')
...
# (re-)make the storage directory for the current batch. 
